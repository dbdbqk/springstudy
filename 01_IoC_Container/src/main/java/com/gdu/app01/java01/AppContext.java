...
public class AppContext {
	...
	*/
	...
	public Contact	contact1() {	// <bean id="contact1" class="Contact">
		...
		return c;					// 반환하는 객체 c가 Spring Container에 저장됩니다.
	}
	...
	public User user1() {			// <bean id="user1" class="user">
		...
		u.setContact(contact1());	// setter <property name="contact" ref="contact1" />    contact1은 위에있는 contact1() 메소드
		return u;					// 반환하는 객체 c가 Spring Container에 저장됩니다.
	}
	
	@Bean(name="contact2")		// name 속성이 추가되면 Bean의 id로 사용됩니다.
	public Contact www() {		// name 속성이 사용되었으므로 메소드명은 더이상 Bean의 id가 아니에요
		return new Contact("02-3333-3333", "02-3333-333");

	}	

	@Bean(name="user2")		// name 속성이 추가되면 Bean의 id로 사용됩니다.
	public User aaa() {		// name 속성이 사용되었으므로 메소드명은 더이상 Bean의 id가 아니에요. 아무 이름이나 사용 가능
