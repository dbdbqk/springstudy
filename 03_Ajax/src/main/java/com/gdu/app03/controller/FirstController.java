...
public class FirstController {
	...
	// FirstServiceImpl()을 Spring Container에 Bean으로 등록시켜보자
	...
	@ResponseBody	// 메소드의 반환 값을 Jsp이름으로 해석하지 않는다. 응답하는 값(데이터)이다.(요청한 곳으로 보내는 데이터)
	...
	public Map<String, Object> ajax2(@RequestParam("name") String name, @RequestParam("age") int age){
		return (firstService.execute2(name, age));		// "Jackson 라이브러리"가 반환값 map을 자동으로 JSON으로 데이터로 변환한다.
	}
