...
public class MvcController {
	...
	*/
	...
	/*
		...
		CharcterEncodingFilter를 통해서 모든 요청마다 자동으로 UTF-8로 인코딩한다.
	...
	*/
	...
	public String detail(HttpServletRequest request, Model model) {
		...
		/*
			...
			2. Model2(Jsp/Servlet)에서는 HttpServletRequest request 객체를 사용해서 데이터를 전달 하지만,
	...
	/*
			2. @Requestparam으로 요청 파라미터 처리하기
				...
				3) defaultValue : 요청 파라미터 없을 때 대신 사용할 값
	 */
	 ...
	 *
	@GetMapping("/detail.me")
	public String detailMe(String name,	// 요청 파라미터 name이 없으면 String name에 경우 null 값이 사용
						   int age,		// 요청 파라미터 age가 없으면 null을 int로 변환 하려고 하기 때문에 오류가 발생
						   Model model) {
	...
	*/
	...
	public String detailGdu(Person p) {	// name과 age를 필드로 가진 커맨드 객체 Person p
										...
										// 객체 타입인 Person을 사용한다. 
										// Model에 저장되는 속셩명은 객체 타입 Person을 person으로 수정해서 사용한다.
		return "mvc/detail";
	}
	/*
	@GetMapping("/detail.gdu")
	public String detailGdu(@ModelAttribute(value="p") Person person) {	// Model에 저장할 속성명을 p로 해 주세요
		return "mvc/detail";											// ${p.name}, ${p.age}와 같은 형식으로 확인할 수 있다.
	}	
	*/
}
